...
//       const resp = await axios.get('/api/company_registration/get-vtp')
//       const users = await axios.get('/api/naqal-kafala')
...
// export default NaqalKafala;

'use client'
import { Box, Heading, Button, Center, Spinner } from "@chakra-ui/react";
import { useEffect, useState } from "react";
import { useDisclosure } from "@chakra-ui/react";
import axios from "axios";
import { HeroSection } from "../../Gamca/MedicalCenters/HeroSection";
...
const NaqalKafala = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const [sliderResponse, agentsResponse, candidatesResponse] = await Promise.all([
          axios.get(`/api/slider?page=NaqalKafala`),
